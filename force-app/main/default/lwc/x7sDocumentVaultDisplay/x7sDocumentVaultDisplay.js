...
export default class X7SDocumentVaultDisplay extends LightningElement {
    ...
    updateVault() {
        //console.log('I am updateVault. I am commented out');
        //this.resetFileListObj();
        this.getFileList();
        /*console.log('entering updateVault-', this.sortedFiles);
        this.resetFileListObj().then((result, error) => {
            if(result){
                console.log('my sorted files - success', this.sortedFiles);
                this.getFileList();
            }
            if(error){
                console.log('my sorted files - error', this.sortedFiles);
                console.error(error);
            }
        });*/  
    }
    ...
    getFileList = () => {
        ...
        getFiles({userId: USER_ID}).then((result, error) => {
            ...
            if(result){
                ...
                this.sortMyFiles(this.resultFiles);
            }
            if(error){
     ...
     sortMyFiles(files){
        ...
        fileList.forEach(file => {
            ...
            if(folder !== undefined && folder !== 'Trash') {
                ...
                recentDocList.push(file);
                fileCount++;
                //console.log('recent docs', recentDocList);
                /* The My Shared Docs folder list is set, so first check to see if the folder exists
                // In this context, if it does, the file should be added to the appropriate folder.
                // If it doesn't, we want to add it to the fromMyAdvisors list. */
                if (this.mySharedDocsFolders.includes(folder)) {
                    ...
                    Object.keys(this.sortedFiles.mySharedDocs.folders).forEach(myFolder => {
                        ...
                        //if (myFolder === folder) {
                        if (myFolder === folder ) {
                            //let folderUnderscored = folder.replace(/ /g,"_");
                            //let thisName = file.Name;
                            //let fileToSort = thisName+folderUnderscored;
                            //console.log('fileSorted', fileToSort);
                            //console.log('HUH', filesSorted.indexOf(fileToSort));
                            //if(filesSorted.indexOf(fileToSort)==-1){
                                //console.log("I should add " + fileToSort);
                                this.sortedFiles.mySharedDocs.folders[myFolder].files.push(file);
                                //filesSorted.push(fileToSort);
                            //}
                            //}else{
                                //console.log("I should NOT add " + fileToSort);
                            //}
                            
                            //
                            //console.log(filesSorted.indexOf(fileSorted));
                        }
                    });
                } else {
                    /* If the folder already exists in the fromMyAdvisorsFolders list, push the file to it
            ...
            //console.log('sorted files', this.sortedFiles);
        });
        setTimeout(() => {
            this.fileCount = fileCount;
            this.sortedFiles.recentDocs.files = recentDocList;
